...
class EnemyMonitorTests(unittest.TestCase):
    ...
    def test_configured_enemy_area_falls_back_to_full_frame_on_miss(self):
    ...
    def create_monitor(
        *,
        ...
        detector = SimpleNamespace(
            detect=lambda image, template: anchor
        )

        resolver = SimpleNamespace(
            resolve=lambda detection, template: object(),
            crop=lambda image, region: np.zeros(
                (2, 2, 3),
                dtype=np.uint8,
            ),
        )
        ...
        monitor = EnemyMonitor(
            ...
            resolver,
            SimpleNamespace(
                read_enemy_hp=lambda image: hp_percent
            ),
            templates,
            name_matcher,
            cache,
    ...
    def test_anchor_search_reuses_the_last_hud_position(self):
        ...
        with patch(
            ...
            first = monitor._detect_anchor(
                image,
                template,
            )
            second = monitor._detect_anchor(
                image,
                template,
            )
    ...
    def test_forced_anchor_recovery_can_leave_the_cached_roi(self):
        ...
        with patch(
            ...
            detection = monitor._detect_anchor(
                image,
                template,
            )
        ...
        self.assertEqual(
            calls[0].args[0].shape[:2],
            (200, 300),
        )
        self.assertFalse(
            monitor.force_full_anchor_search
        )
    ...
    def test_stale_worker_result_never_writes_enemy_or_item_database(self):
        for (
            ...
            (
                EnemyMonitor.ENTITY_ENEMY,
                EnemyMonitor.ENTITY_ITEM,
                1,
            ),
            (
                EnemyMonitor.ENTITY_ITEM,
            ...
            with self.subTest(
                ...
                monitor.selection_id = (
                    current_selection
                )
                monitor.current_entity_type = (
                    current_type
                )
                monitor.identity_future = (
                    CompletedFuture(
    ...
    def test_name_without_hp_bar_is_stored_as_item_after_acquisition_grace(self):
        database = SimpleNamespace(
            resolve_enemy_name=MagicMock(
                side_effect=AssertionError(
                    "no debe resolver enemigo"
                )
            ),
            register_enemy_seen=MagicMock(),
            resolve_item_name=MagicMock(
                return_value="Healing Potion"
            ),
            register_item_seen=MagicMock(
                return_value=True
            ),
        ...
        self.assertEqual(target.name, "")
        self.assertEqual(
            cache.current_enemy_name,
            "",
        )

        database.resolve_item_name.assert_called_once_with(
            "Healing Potion"
        )
        database.register_item_seen.assert_called_once_with(
            "Healing Potion"
        )
        database.register_enemy_seen.assert_not_called()
    ...
    def test_zero_reader_result_stays_unknown_until_a_valid_hp_arrives(self):
        ...
        monitor.bar_reader.read_enemy_hp = (
            lambda image: 50
        )
        ...
        database.resolve_enemy_name.assert_called_once_with(
            "Bongbo"
        )
        database.register_enemy_seen.assert_called_once_with(
            "Bongbo"
        )
        database.resolve_item_name.assert_not_called()
    ...
    def test_invalid_hp_after_measurement_preserves_presence_and_last_value(self):
        ...
        monitor.target_validator.has_red_bar = (
