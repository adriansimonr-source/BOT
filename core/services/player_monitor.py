...
class PlayerMonitor:
    ...
    def update(self, image, player_state: PlayerState):
        ...
        self._reset_resource_misses()
        resource_status = self.read_resources(hud_image, player_state)
    ...
    def _reacquire_due(self, resource_status):
        ...
        if not hp_updated and not mp_updated:
            return max(self.hp_misses, self.mp_misses) >= (
    ...
    def _record_resource_status(self, resource_status):
        hp_updated, mp_updated = resource_status
        self.hp_misses = 0 if hp_updated else self.hp_misses + 1
        self.mp_misses = 0 if mp_updated else self.mp_misses + 1
    ...
    def _crop_hud(self, image, hud):
        crop = self.resolver.crop(image, hud)
        if crop is None:
            return None
        if isinstance(hud, dict):
            expected_height = max(0, int(hud.get("height", 0)))
            expected_width = max(0, int(hud.get("width", 0)))
            if crop.shape[:2] != (expected_height, expected_width):
                return None
        return crop

    @staticmethod
    def crop_region(image, region):
        if image is None or region is None or not hasattr(image, "shape"):
            return None
